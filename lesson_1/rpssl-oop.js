...
const RPSSLGame = {
  ...
  playOne() {
    // this ensures that the computer's first move is random
    // if no move history yet exists
...
function createComputer() {
  ...
  let computerObject = {
    ...
    chooseAdvanced(obj) {
      ...
      // determine probabilities of win for each choice
      // assuming continuation of historic pattern
      let winProbs = [
      ...
      let index = winProbs.findIndex((prob, _, arr) => {
        return prob === Math.max(...arr);
      });
...
function createHuman() {
  ...
  let humanObject = {
    choose(obj) {
      ...
      while (!(Number(choice) >= 1
        && Number(choice) <= 5) || choice.length > 1) {
