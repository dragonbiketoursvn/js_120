...
It should never display the goodbye message before that.
  */
  ...
  joinOr(unusedSquares, delimiter = ", ", penultimate = "or") {
    if (unusedSquares.length === 1) {
      return unusedSquares[0];
    } else if (unusedSquares.length === 2) {
      return `${unusedSquares[0]} ${delimiter} ${unusedSquares[1]}`;
    } else {
      return unusedSquares.reduce((prev, curr, idx, arr) => {
        if (idx === 0) {
          return prev.concat(`${curr}`);
        } else if (idx === arr.length - 1) {
          return prev.concat(`${delimiter} ${penultimate} ${curr}`);
        } else {
